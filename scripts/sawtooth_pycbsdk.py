...
class DummyApp:
    ...
    def handle_frame(self, pkt):
        if self._write_index < self._buffer.shape[0]:
            ...
            if self._write_index > 0 and self._ts[self._write_index] != (self._ts[self._write_index - 1] + 1):
                logger.warning(
                    f"Duplicate timestamp detected at index {self._write_index - 1}: {self._ts[self._write_index - 1]}"
                )
            self._write_index += 1
